    ...
    <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <div class="p-6 space-y-6">
                <form action="{{ route('stock.opname.submit') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <select name="product_id" id="product_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                ...
                                <option value="{{ $product->id }}" data-category="{{ $product->category->name }}" data-sku="{{ $product->sku }}" data-stock="{{ $product->stock }}">{{ $product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-span-6 sm:col-span-3">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            <label for="stock" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Stock Awal</label>
...
<script>
    document.getElementById('product_id').addEventListener('change', function() {
        ...
        if (selectedOption.value) { 
            ...
            document.getElementById('sku').value = selectedOption.getAttribute('data-sku');
